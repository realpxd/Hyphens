...
function pushing () {
  ...
  function selectedAns (currPos) {
    ...
    function saveFreq(updatedQuiz){
      localStorage.setItem("updatedQuiz", JSON.stringify(updatedQuiz));
      console.log(localStorage.getItem("updatedQuiz"));
      console.log("stored\n\n\n\n");
    }
    
    saveFreq(updatedQuiz);
    
    return quizData , updatedQuiz;
    // , len , currPos;
  }

  
  // setInterval(saveFreq, 1000)
...
clearQuizBtn.addEventListener("click", e => {
    var confirmPrompt = confirm("Are you sure you want to clear all local data?");
    if (confirmPrompt) {
      localStorage.clear();
